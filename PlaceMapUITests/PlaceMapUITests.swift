...
class PlaceMapUITests: XCTestCase {
    ...
    func testExample() {
        ...
        // Use XCTAssert and related functions to verify your tests produce the correct resUults.
        ...
        let c = map.coordinate(withNormalizedOffset: CGVector(dx: 0.0, dy: 0.0)).screenPoint
        XCTAssertEqual(c, CGPoint(x: 16.0, y: 72.0))

        app.navigationBars.buttons.matching(identifier: "Back").element(boundBy: 0).tap();
        XCTAssertEqual(app.tables.count, 1)
        ...
        let prefecturalCapitalButton = app.navigationBars.matching(identifier: "PlaceMap.MapDisplayView").buttons["Prefectural Capital"]
