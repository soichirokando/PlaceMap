...
class MapDisplayViewController: UIViewController {
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        if let prefName = places[selectedIndex]["pref"],
            let capName = places[selectedIndex]["name"],
            let lat = places[self.selectedIndex]["latitude"],
            let lon = places[self.selectedIndex]["longitude"] {
                self.placeName.text = "\(prefName) [\(capName)]"
                if let wLat = CLLocationDegrees(lat), let wLon = CLLocationDegrees(lon) {
